...
import { generateRoundId, createRoundEvent, createPlayerEvent } from '@/lib/golf/nostrEvents';
...
const isHexPubkey = (value: string) => /^[a-fA-F0-9]{64}$/.test(value);
...
interface PlayerRowProps {
  ...
  validatedPubkey: string | null;
  pubkeyError: string | null;
  isValidating: boolean;
  onUpdatePlayer: (index: number, field: keyof PlayerInRound, value: unknown) => void;
  onRemovePlayer: (index: number) => void;
  onApplyProfileData: (index: number, update: PlayerProfileUpdate) => void;
...
const PlayerRow: React.FC<PlayerRowProps> = ({
  ...
  const isCurrentUser = player.playerId === currentUserPubkey;
  const profilePubkey = validatedPubkey || (isHexPubkey(player.playerId) ? player.playerId : '');
  const { data: profileData } = useAuthor(profilePubkey);
  const profileMetadata = profileData?.metadata;

  const hasNostrProfile = Boolean(profilePubkey && profileMetadata);
  const displayName = isCurrentUser
    ? (player.name || currentUserMetadata?.name || currentUserMetadata?.display_name || (currentUserPubkey ? genUserName(currentUserPubkey) : ''))
    : hasNostrProfile
      ? (profileMetadata?.name || profileMetadata?.display_name || genUserName(profilePubkey))
      : player.name;
  ...
  useEffect(() => {
    if (!profilePubkey || !profileMetadata) return;
    if (player.playerId !== profilePubkey) return;

    const actualName = profileMetadata.name || profileMetadata.display_name || genUserName(profilePubkey);
    const profileHandicap = getGolfHandicap(profileMetadata) || 0;
    const updates: PlayerProfileUpdate = {};

    if (actualName && player.name !== actualName) {
      updates.name = actualName;
    }

    if ((player.handicap || 0) === 0 && profileHandicap > 0) {
      updates.handicap = profileHandicap;
    }

    if (Object.keys(updates).length > 0) {
      onApplyProfileData(index, updates);
    }
  }, [profilePubkey, profileMetadata, player.playerId, player.name, player.handicap, index, onApplyProfileData]);
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {isCurrentUser || hasNostrProfile ? (
            ...
            <div className="relative">
              <Input
                ...
                className={`h-8 ${pubkeyError ? "border-destructive" : ""}`}
              />
              {isValidating && (
          ...
          {pubkeyError && (
            <p className="text-xs text-destructive mt-1">{pubkeyError}</p>
          )}
        </div>
        ...
        <div className="w-16 flex-shrink-0">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              onUpdatePlayer(index, 'handicap', Number.isNaN(value) ? 0 : value);
            }}
...
export const RoundSetupPage: React.FC = () => {
  ...
  const [round, setRound] = useState<Partial<GolfRound>>(() => ({
    id: urlRoundId ? undefined : generateRoundId(), // Don't set ID yet if loading from URL
    date: Date.now(),
    players: [],
  ...
  const [selectedGameModeOrder, setSelectedGameModeOrder] = useState<(keyof GameModes)[]>([]);
  ...
  const [loadingRound, setLoadingRound] = useState<boolean>(Boolean(urlRoundId));

  const { nostr } = useNostr();

  const generateRoundCode = useCallback(async () => {
    ...
    try {
      // Ensure round ID is set
      const actualRoundId = round.id || generateRoundId();
      ...
      // Update local round state with the actual ID
      setRound(prev => ({ ...prev, id: actualRoundId }));

      const roundEvent = createRoundEvent(roundToPublish, code);
      await publishEvent(roundEvent);

      // Also publish the current user as a player if not already in the round
      const isHostInRound = (round.players || []).some(p => p.playerId === user.pubkey);
      if (!isHostInRound) {
    ...
    setHasGeneratedRoundCode(true);
  }, [user, round, selectedCourse, metadata, publishEvent, hasGeneratedRoundCode, toast]);

  // Auto-generate round code on load (only for new rounds, not join flows)
  useEffect(() => {
    // Only generate if:
    // 1. User is logged in
    // 2. Not loading from a URL (not a join flow)
    // 3. Haven't already generated a code
    if (user && !urlRoundId && !hasGeneratedRoundCode && nostr) {
      generateRoundCode();
    }
  }, [user, urlRoundId, hasGeneratedRoundCode, nostr, generateRoundCode]);

  // If the URL contains ?roundId=..., attempt to load that round from Nostr relays
  useEffect(() => {
    if (!urlRoundId) return;

    // If the current round already matches the requested id and has players, skip reload
    ...
    (async () => {
      ...
      try {
        ...
        // Query for round events by EITHER:
        // 1. Direct d tag match (for rounds not created via join code)
        // 2. round-id tag match (for rounds created via join code, where d tag is "join-CODE")
        const filters = [
          { kinds: [APP_KIND], '#d': [urlRoundId], '#t': ['golf', SUBTYPES.ROUND], limit: 1 },
          { kinds: [APP_KIND], '#round-id': [urlRoundId], '#t': ['golf', SUBTYPES.ROUND], limit: 1 },
        ...
        // Find round event by either d tag or round-id tag
        const roundEvent = events.find((e: NostrEvent) => {
          ...
          // Check if d tag matches OR round-id tag matches
  ...
  // Subscribe to player events for real-time multiplayer sync
  useEffect(() => {
    ...
    const subscribeToPlayers = async () => {
      try {
        ...
        for await (const msg of subscription) {
          ...
          if (msg[0] === 'CLOSED') {
            console.log('[RoundSetup] Player subscription closed');
            break;
          }
          ...
          if (msg[0] === 'EVENT') {
            ...
            // Add or update player in real-time
            setRound(prev => {
              const existingPlayers = prev!.players || [];
              const existingIndex = existingPlayers.findIndex(p => p.playerId === playerId);
              
              if (existingIndex >= 0) {
                // Update existing player (preserve local edits like scores)
                return prev;
              } else {
                // Add new player
                const newPlayer: PlayerInRound = { playerId, name, handicap, scores: [], total: 0, netTotal: 0 };
                return { ...prev!, players: [...existingPlayers, newPlayer] };
              }
            });
          }
        }
        ...
        console.error('[RoundSetup] Player subscription error:', error);
      }
    };
  ...
  const acceptInvite = async () => {
    ...
    const existingPlayer = (round.players || []).find(p => p.playerId === user.pubkey);
    if (existingPlayer) {
      toast({ title: 'Already joined', description: 'You are already in this round' });
      return;
    }
    ...
    try {
      ...
      setRound(prev => ({ ...prev!, players: (prev!.players || []).filter(p => p.playerId !== user.pubkey) }));
  ...
  const useHandicaps = useMemo(
    () => (round.players || []).some(p => (p.handicap || 0) > 0),
    [round.players]
  );
  const [netScoring] = useState<boolean>(false);
  const [wolfPartners] = useState<Record<string, string | null>>({});
  ...
  useEffect(() => {
    ...
    setRound(prev => {
      ...
      if (existingUserIndex >= 0) {
        ...
        if (existing && existing.name === userName) {
          // No change needed
          return prev;
        }
        // Update only the name, keep handicap as-is
        const updatedPlayers = [...currentPlayers];
        updatedPlayers[existingUserIndex] = { ...existing, name: userName } as PlayerInRound;
  ...
  const applyProfileDataToPlayer = useCallback((index: number, update: PlayerProfileUpdate) => {
    setRound(prev => ({
      ...prev!,
  ...
  const guardKey = urlRoundId || round.id || 'new-round';
  ...
  }, [guardKey, loadingRound, serializedGuardState]);
  ...
  const getPubkeyFromInput = (input: string): string | null => {
  ...
  const validateNpubInput = async (input: string, playerIndex: number) => {
    ...
    try {
      const pubkey = getPubkeyFromInput(trimmedValue);
      if (!pubkey) {
        setValidatedPubkeys(prev => ({ ...prev, [playerIndex]: null }));
        setValidatingPlayers(prev => ({ ...prev, [playerIndex]: false }));
        return;
  ...
  const updatePlayer = (index: number, field: keyof PlayerInRound, value: unknown) => {
    ...
    if (field === 'name') {
      ...
      try {
        parsedPubkey = getPubkeyFromInput(nameValue);
      } catch {
        parsedPubkey = null;
      }

      const actualName = parsedPubkey ? genUserName(parsedPubkey) : nameValue;
      const playerId = parsedPubkey ? parsedPubkey : `player-${index + 1}`;
  ...
  const startRound = () => {
    ...
    const roundId = round.id || generateRoundId();
    ...
    const players = (round.players || []).map(player => ({
      ...player,
      scores: player.scores?.length ? player.scores : Array(holes.length).fill(0)
    }));
    ...
    const roundToPublish: GolfRound = {
      ...
      status: 'active',
      metadata: {
        courseName: round.metadata?.courseName || selectedCourse?.name || '',
        courseLocation: round.metadata?.courseLocation || selectedCourse?.location || '',
        teeBox: round.metadata?.teeBox || '',
        teeYardage: round.metadata?.teeYardage,
        weather: round.metadata?.weather || '',
        notes: round.metadata?.notes || '',
        selectedSection: round.metadata?.selectedSection,
      }
    };

    setRound(roundToPublish);

    const proceedToScore = () => {
      guardSnapshotRef.current = JSON.stringify({
        ...guardState,
        round: roundToPublish,
        selectedCourseId: selectedCourse?.id || null,
      });
      setIsDirty(false);

      navigate(`/round/${roundId}/score`, {
        state: {
          round: roundToPublish,
          selectedCourse,
          selectedGameModes,
          selectedGameModeList,
          wagersEnabled,
          wagerAmounts,
          customWagerAmounts,
          pinseekrWagersEnabled,
    ...
    const roundEvent = createRoundEvent(roundToPublish, roundCode || undefined);
    publishEvent(roundEvent, {
      ...
      onError: () => {
        toast({
          ...
          description: 'Failed to publish the round. Continue without saving?',
          variant: 'destructive'
        });
        const shouldContinue = window.confirm('Round publish failed. Continue without saving?');
        if (shouldContinue) {
          proceedToScore();
        }
      }
              ...
              <Card className="bg-transparent border-0 shadow-none">
                ...
                <CardContent>
                  <div className="space-y-8">
                        ...
                        {round.players && round.players.length > 0 ? (
                          round.players.map((player, index) => (
                            <PlayerRow
                              ...
                              currentUserMetadata={metadata || undefined}
                              validatedPubkey={validatedPubkeys[index] || null}
                              pubkeyError={pubkeyErrors[index] || null}
                              isValidating={validatingPlayers[index] || false}
                              onUpdatePlayer={updatePlayer}
                              onRemovePlayer={removePlayer}
                              onApplyProfileData={applyProfileDataToPlayer}
                    ...
                    {/* Round Code Display - Always visible when code exists */}
                    {roundCode && (
                      <div className="p-4 bg-gradient-to-r from-yellow-50 to-orange-50 dark:from-yellow-950 dark:to-orange-950 border-2 border-yellow-300 dark:border-yellow-700 rounded-lg shadow-sm">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            <QrCode className="h-5 w-5 text-yellow-700 dark:text-yellow-300" />
                            <h4 className="font-semibold text-yellow-800 dark:text-yellow-200">Round Join Code</h4>
                          </div>
                          <div className="text-2xl font-mono font-bold text-yellow-900 dark:text-yellow-100 bg-white dark:bg-gray-800 px-4 py-2 rounded border-2 border-yellow-400 dark:border-yellow-600">
                            {roundCode}
                          </div>
                        </div>
                        <p className="text-sm text-yellow-700 dark:text-yellow-300 mb-3">
                          Share this code with friends to join your round. They can enter it at pinseekr.golf or scan the QR code.
                        </p>
                          ...
                          {selectedCourse?.tees && selectedCourse.tees.length > 0 ? (
                            <Select
                              onValueChange={(value) => {
                                ...
                                const yardage = parseInt(yardageStr || '0', 10) || 0;
                                setRound(prev => ({
                                  ...prev!,
...
// SCORING SECTION REMOVED - Lives in RoundScorePage_v2.tsx
// Settlement handlers (generateAndPublishSettlement, confirmSettlementAndCreateInvoices)
// will be moved to scoring page when that component is fully ported.
